...
test('registers all seven native tools and returns JSON strings', async () => {
  ...
  assert.equal(registrations.find(tool => tool.name === 'reddit_delete')!.annotations.consequentialHint, true);
  assert.equal(registrations[1].annotations.untrustedContentHint, true);
  assert.equal(installRedditWebMCP({ window: win }), api);
  assert.equal(registrations.length, 7);
  api.dispose();
  assert.equal(aborted.length, 7);
...
test('registration failure preserves local tools and reports the failure', async () => {
  ...
  const api = installRedditWebMCP({ window: win }); await api.ready;
  assert.equal(api.status().registrationErrors.length, 7);
